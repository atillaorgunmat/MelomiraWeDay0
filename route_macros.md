# route_macros.md — paste-ready snippets

- Start L0:
  FOUNDATIONS_GATE = OPEN
  ROUTE → GUIDANCE‑ORG (FREE) for P‑FND‑0001: task=CLARIFIER_ROUND level=L0 mode=ROOT_FINDER
...
- To PRO‑EVAL:
  ROUTE → PRO‑EVAL (FREE) for P‑FND‑0001

- Method Snapshot:
  FOUNDATIONS_GATE = OPEN
  ROUTE → GUIDANCE‑ORG (FREE) for P‑FND‑0001: task=CLARIFIER_ROUND level=L0 (Method Snapshot)
